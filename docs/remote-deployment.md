...
- 浏览器播放分析视频（文搜视频页主播放器与以图搜图/快速布防弹窗）一律走 backend-lite `GET /api/video-analysis/video?videoUrl=`（同源 5173 反代，Range 透传支持 seek），**不要把 MinIO 文件地址直接塞进 `<video>`**：现场客户端链路直连 9000 被限速（2026-09-24 实测仅 ~7KB/s，370MB 录像首帧要等数分钟；经 5173 反代首字节 <10ms）。框选确认图直接从弹窗定格帧 canvas 裁出上传，无需再调截帧接口。
...
- 环境变量只在容器创建时注入，改 `.env` 或 compose 的 `environment` 后必须重建容器才生效。2026-09-24 实测 10.10 现场 `videoai-backend-1` 是旧配置创建的，容器内完全没有 `MINIO_*`（`docker exec videoai-backend-1 env | grep -i minio` 为空），backend-lite 回落到代码默认值 `192.168.11.194` 导致「上传本地视频」失败；`cd /home/public/videoai && docker compose up -d --no-deps backend` 重建后注入 `MINIO_ENDPOINT=10.10.3.100` 恢复。注意 8081 只绑定宿主机 `10.10.3.100`，宿主机上 `curl 127.0.0.1:8081` 会连接拒绝，需用 `http://10.10.3.100:8081` 验证。
