...
export default defineComponent({
  ...
  methods: {
    ...
    applyCameras(list: any[]) {
      ...
      for (const c of list) {
        const areaPath = (c.area || "").trim() || "未分配";
      ...
      if (list.length) {
        this.feeds = list.slice(0, 4).map((c: any) => ({
          name: c.name,
          meta: `${c.protocol || c.streamName || "IPC"} · ${statusLabel(c.status)} · 主码流`,
          image: this.store.img.car,
          camera: c
        }));
        this.selectedFeed = 0;
