  ...
  <section class="content review-wide media-preview-page">
    ...
    <div class="media-console-grid" :class="{ 'ptz-hidden': !ptzSupported }">
      <aside class="panel media-resource-panel">
        ...
        <div v-if="activeResourceTab === 'monitor'" class="media-playback-tree"><div class="media-panel-head"><b>监控资源</b><span class="hint-text">区域 / 监控点 · 共 {{ totalCameraCount }} 台设备</span></div><div class="media-playback-tree-list exact-tree-list"><div v-for="region in displayRegions" :key="region.fullPath"><button class="exact-tree-area-row" :class="{ active: selectedRegion && selectedRegion.fullPath === region.fullPath }" :style="{ paddingLeft: (8 + region.depth * 16) + 'px' }" @click="toggleRegion(region)"><span>{{ isRegionExpanded(region) ? '⌄' : '›' }} {{ region.name }}</span><span>{{ regionCount(region) }} 台设备</span></button><div v-if="isRegionExpanded(region)" class="exact-tree-children" :style="{ marginLeft: (18 + region.depth * 16) + 'px' }"><button v-for="camera in camerasForRegion(region)" :key="camera.code" class="exact-tree-device" :class="{ active: selectedCamera && selectedCamera.code === camera.code }" draggable="true" title="点击放入当前窗口，或拖拽到目标窗口" @dragstart="onCameraDragStart($event, camera)" @click="selectCamera(camera, region)"><span class="media-fav-star" :class="{ 'is-favorite': isFavorite(camera.code) }" :title="isFavorite(camera.code) ? '取消收藏' : '收藏'" @click.stop="toggleFavorite(camera.code)">{{ isFavorite(camera.code) ? '★' : '☆' }}</span><span class="media-device-name">{{ camera.name }}</span><span class="media-tree-status" :class="deviceStatusClass(camera.camera)">{{ camera.status }}</span></button></div></div><div v-if="!displayRegions.length" style="padding:12px;color:#888;">{{ searchKeyword ? '无匹配监控点' : '暂无监控点，请先在设备管理中添加设备' }}</div></div></div>
        <ul v-else-if="activeResourceTab === 'favorite'" class="media-resource-list"><li class="group">我的收藏</li><li v-for="camera in favoriteCameras" :key="camera.code" style="cursor:pointer;" draggable="true" title="点击放入当前窗口，或拖拽到目标窗口" @dragstart="onCameraDragStart($event, camera)" @click="selectCameraById(camera.code)"><span class="online-dot media-fav-star is-favorite">★</span>{{ camera.name }}<span class="link-red" style="margin-left:auto;" @click.stop="toggleFavorite(camera.code)">取消</span></li><li v-if="!favoriteCameras.length" style="color:#888;">暂无收藏，在监控点列表点击 ☆ 收藏</li></ul>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      startFailNotified: new Set<string>(),
      cameraList: [] as any[],
      regions: [] as RegionNode[],
      regionCameras: {} as Record<string, any[]>,
      expandedRegions: {} as Record<string, boolean>,
      feeds: [] as any[],
      // 每路窗口已加载视频的宽高比（"1920 / 1080"），用于窗格高度适配视频
      videoAspects: {} as Record<number, string>,
  ...
  methods: {
    ...
    // 录像回放页“切换实况”带入的设备（state.playCameraId）：上屏到当前选中窗口直接播放
    playPendingCamera() {
      const id = this.state.playCameraId;
      if (!id) return;
      this.state.playCameraId = "";
    ...
    buildFeed(camera: any) {
      // 码流默认跟随新增/编辑设备时配置的「码流类型」（主码流/子码流）；
      // 设备无子码流代理或开启目标检测标注时回退主码流
      const preferSub = camera.nvrStreamType === "子码流" || camera.nvrStreamType === "sub";
      const feed = {
        name: camera.name,
        meta: "",
        camera,
        streamType: preferSub && camera.subStreamName && !camera.objectDetectionEnabled ? "sub" : "main",
        digitalZoom: false
      };
          ...
          api.startCamera(c.id)
            .then((started: any) => {
              if (started) {
                ...
                this.startFailNotified.delete(c.id);
              }
            })
            .catch((error: any) => {
              // 拉起失败按设备去重提示一次，避免多窗口/定时刷新重复弹 toast
              if (!this.startFailNotified.has(c.id)) {
                this.startFailNotified.add(c.id);
                this.showToast(`设备「${c.name}」拉起失败：${(error && error.message) || "未知原因"}`);
              }
            })
    ...
    // 视频元数据就绪：记录该窗口视频宽高比，窗格高度随宽度按视频比例收缩
    onVideoResolution(index: number, resolution: { width: number; height: number }) {
    ...
    selectCamera(camera: any, region: RegionNode | null) {
      ...
      if (!this.checkCameraPlayable(realCamera)) return;
    ...
    playCameraAt(camera: any, index: number) {
      if (!this.checkCameraPlayable(camera)) return;
    ...
    // 即时回放：传入当前选中窗口的设备，弹窗内按 quickReplaySeconds 回放近期录像
    openQuickReplay() {
      const feed = this.visibleFeeds[this.selectedFeed];
      if (!feed || !feed.camera) {
        this.showToast("当前窗口无视频流");
