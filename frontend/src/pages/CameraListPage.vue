      ...
      <div class="video-device-main">
        <div class="review-board video-device-board">
          ...
          <div class="video-device-toolbar">
            <div class="video-device-toolbar-actions"><button class="btn primary" @click="setRoute('mediaDeviceWizard')">＋ 新增</button><button class="btn" @click="openModal('mediaCloud')">云平台同步</button><button class="btn" @click="openModal('mediaNvrImport')">⤓ 从NVR/CVR导入</button><button class="btn" @click="openModal('mediaImport')">⇧ 批量导入</button><button class="btn" @click="exportDevices">⇩ 导出</button><button class="btn" @click="moveSelected">⇄ 批量设备移动</button><button class="btn" @click="openCapability">⚙ 能力配置</button><button class="btn" @click="openModal('mediaRegion')">▦ 区域管理</button><button class="btn danger" @click="deleteSelected">删除</button></div>
